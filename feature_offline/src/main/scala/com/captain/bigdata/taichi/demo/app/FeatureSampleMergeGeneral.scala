...
object FeatureSampleMergeGeneral {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (filterLowUser) {
      val filterSql = "select a.* from TMP_TBL_01 a join cmp_tmp.cmp_tmp_user_ctr b on a.dt = b.dt and upper(a.device_id) = upper(b.device_id) and b.click_num > 0 and brush_num < 20"
      println("filterSql:" + filterSql)
      dataFrame = spark.sql(filterSql)
    }
